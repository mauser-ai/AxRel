...
class NS_Bridge_Frontend {
	...
	public static function filter_loop_add_to_cart_link($html, $product) {
		...
		return $url
			? sprintf('<a href="%s" class="button ns-bridge-buy-on-shopify">Acquista su Shopify</a>', esc_url($url))
			: $html;
	}
	...
	public static function render_buy_on_shopify() {
		...
		$base = self::cart_base_url();
		if (!$base) {
			echo '<p class="ns-bridge-buy-warning">Dominio Shopify non configurato: impossibile generare il link di acquisto.</p>';
			return;
		}
		...
		$url = self::simple_checkout_url($product);
		if (!$url) {
			echo '<p class="ns-bridge-buy-warning">Variante Shopify non disponibile per questo prodotto.</p>';
			return;
		}
		printf(
			'<a href="%s" class="single_add_to_cart_button button alt ns-bridge-buy-on-shopify">Acquista su Shopify</a>',
			esc_url($url)
		);
	}
	...
	private static function render_variable_buy_button($product, $base) {
		...
		$container_id = 'ns-bridge-buy-' . $product->get_id();
		?>
		<div id="<?php echo esc_attr($container_id); ?>" class="ns-bridge-variant-buy">
			<?php foreach ($variation_options as $slug => $options) :
				...
				<p style="margin-bottom:.75em;">
					...
					<select class="ns-bridge-attribute" data-attribute="<?php echo esc_attr($slug); ?>">
			...
			<?php endforeach; ?>
			<a href="#" class="single_add_to_cart_button button alt ns-bridge-buy-on-shopify" aria-disabled="true" style="pointer-events:none;opacity:.5;">
				Acquista su Shopify
			</a>
		</div>
		<script>
		(function () {
			...
			var selects = root.querySelectorAll('select.ns-bridge-attribute');
			var link = root.querySelector('a.ns-bridge-buy-on-shopify');
